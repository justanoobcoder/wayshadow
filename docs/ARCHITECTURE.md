...
### 2.1 `wayshadow::core` (`color`, `config`, `buffer`, `state`)
- **`Color`**: Strong value-type representation of RGBA values with `constexpr` palette constants and hex parser (`#RRGGBB` / `#RRGGBBAA`).
- **`Config`**: Strong configuration settings loaded from `~/.config/wayshadow/wayshadow.conf` or command-line arguments.
...
### 2.2 `wayshadow::input` & `wayshadow::xkb` (`input`, `xkb_handler`)
...
### 2.3 `wayshadow::render` (`draw`, `icons`, `shm`)
...
### 2.4 `wayshadow::wayland` & `wayshadow::ui` (`wl_setup`, `window`, `tray`, `app`)
