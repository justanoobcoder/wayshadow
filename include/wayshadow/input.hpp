#ifndef WAYSHADOW_INPUT_HPP
#define WAYSHADOW_INPUT_HPP
...
class InputManager {
 public:
  ...
  using MouseButtonCallback =
    std::function<void(uint32_t button, uint32_t state)>;
  using MouseMotionCallback = std::function<void(double dx, double dy)>;

  InputManager(KeyCallback key_callback, MouseButtonCallback button_callback,
               MouseMotionCallback motion_callback, ClientState& state);
  ...
  [[nodiscard]] bool IsValid() const noexcept { return li_ != nullptr; }
