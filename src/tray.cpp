...
void TrayIcon::UpdateToggleLabel() {
  if (!toggle_item_ || !state_) return;
  const char* label = state_->overlay_enabled ? "\xe2\x98\x91 Show && hide"
                                              : "\xe2\x96\xa1 Show && hide";

  // clang-format off
  gtk_menu_item_set_label(GTK_MENU_ITEM(toggle_item_), label);  // NOLINT(bugprone-casting-through-void)
  // clang-format on
}

void TrayIcon::OnToggleActivate(GtkMenuItem* /*item*/, void* data) {
  auto* self = static_cast<TrayIcon*>(data);
  if (!self || !self->state_ || !self->win_mgr_) return;

  self->state_->overlay_enabled = !self->state_->overlay_enabled;
  self->UpdateToggleLabel();

  if (!self->state_->overlay_enabled) {
    self->win_mgr_->HideWindow(*self->state_);
  }
}

void TrayIcon::OnExitActivate(GtkMenuItem* /*item*/, void* data) {
  auto* self = static_cast<TrayIcon*>(data);
  if (!self || !self->state_) return;
...
bool TrayIcon::Init() {
  ...
  indicator_ = app_indicator_new("wayshadow-tray", "input-keyboard",
  ...
  if (exe_len > 0) {
    ...
    for (const auto& candidate : {
           ...
           exe_dir.parent_path() / "share" / "icons" / "hicolor" / "scalable" /
             "apps" / "way_shadow.svg",
         }) {
  ...
  if (icon_path.empty()) {
    ...
    if (getcwd(cwd, sizeof(cwd))) {
      for (const auto& candidate : {
             ...
             std::filesystem::path(cwd).parent_path() / "public" /
               "way_shadow.svg",
           }) {
        if (std::filesystem::exists(candidate)) {
          icon_path = candidate.string();
  ...
  g_signal_connect(toggle_item_, "activate", G_CALLBACK(OnToggleActivate),
                   this);

  // clang-format off
  gtk_menu_shell_append(GTK_MENU_SHELL(menu_), toggle_item_);  // NOLINT(bugprone-casting-through-void)

  GtkWidget* quit_item = gtk_menu_item_new_with_label("Exit");
  g_signal_connect(quit_item, "activate", G_CALLBACK(OnExitActivate), this);
  gtk_menu_shell_append(GTK_MENU_SHELL(menu_), quit_item);  // NOLINT(bugprone-casting-through-void)

  gtk_widget_show_all(menu_);
  app_indicator_set_menu(indicator_, GTK_MENU(menu_));  // NOLINT(bugprone-casting-through-void)
  // clang-format on
