...
XkbHandler::XkbHandler() {
  ...
  if (ctx_) {
    map_ =
      xkb_keymap_new_from_names(ctx_, nullptr, XKB_KEYMAP_COMPILE_NO_FLAGS);
    if (map_) {
...
const char* XkbHandler::GetKeySymbol(xkb_keysym_t keysym) noexcept {
  switch (keysym) {
    case XKB_KEY_Return:
...
bool XkbHandler::IsModifier(xkb_keysym_t key) noexcept {
...
void XkbHandler::ProcessRepeatKey(ClientState& state, uint32_t key) {
  ProcessKeyDown(state, key);
}

void XkbHandler::ProcessKeyDown(ClientState& state, uint32_t key) {
  ...
  const bool is_mod = IsModifier(keysym);
  ...
  if (state.overlay_enabled) {
    ...
    if (keysym == XKB_KEY_BackSpace) {
      if (state.modifiers.ctrl) {
        state.buffer.ResetRepeatState();
        state.buffer.DeleteWord();
      } else {
        state.buffer.AppendOrIncrement("\xe2\x8c\xab");  // ⌫ backspace glyph
      }
      ...
      std::string combined;
      if (state.modifiers.ctrl) combined += "Ctrl+";
      if (state.modifiers.alt) combined += "Alt+";
      if (state.modifiers.super) combined += "Super+";

      const char* sym = GetKeySymbol(keysym);
      char key_str[32] = {0};
      ...
      if (sym) {
        ...
        if (std::strlen(key_str) == 0 ||
            static_cast<unsigned char>(key_str[0]) < 32) {
      ...
      if (!combined.empty()) {
        ...
        if (state.modifiers.ctrl && !state.modifiers.alt &&
          ...
          if (keysym == XKB_KEY_c || keysym == XKB_KEY_v ||
            ...
            state.current_combo_color = colors::kComboGreen;
            state.use_combo_color = true;
          } else {
            state.current_combo_color = colors::kComboBlue;
            state.use_combo_color = true;
          }
        } else if (state.modifiers.alt && !state.modifiers.ctrl) {
          state.current_combo_color = colors::kComboPurple;
          state.use_combo_color = true;
        } else if (state.modifiers.super) {
          state.current_combo_color = colors::kComboOrange;
          state.use_combo_color = true;
        }

        state.buffer.AppendOrIncrement(combined);
      }
    }
...
void XkbHandler::HandleKeyEvent(ClientState& state, uint32_t key,
                                  uint32_t state_val) {
  const uint32_t xkb_keycode = key + 8;
  const xkb_keysym_t raw_sym = xkb_state_key_get_one_sym(state_, xkb_keycode);
  const bool is_mod_key = IsModifier(raw_sym);
  ...
  if (state_val == LIBINPUT_KEY_STATE_PRESSED) {
    if (is_mod_key) {
      if ((raw_sym == XKB_KEY_Shift_L || raw_sym == XKB_KEY_Shift_R) &&
        ...
        return;
      if ((raw_sym == XKB_KEY_Control_L || raw_sym == XKB_KEY_Control_R) &&
    ...
    const xkb_keysym_t keysym = xkb_state_key_get_one_sym(state_, xkb_keycode);
    if (!IsModifier(keysym) && state.repeat_rate > 0 &&
