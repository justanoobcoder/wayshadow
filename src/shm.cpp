...
namespace {
[[nodiscard]] int CreateShmFile() noexcept {
  static unsigned int counter = 0;
  char name[32];
  std::snprintf(name, sizeof(name), "/wl_shm-%u-%u",
                static_cast<unsigned int>(getpid()), counter++);
...
int ShmPool::AllocateShmFile(size_t size) noexcept {
  const int fd = CreateShmFile();
  if (fd < 0) {
    return -1;
  }
