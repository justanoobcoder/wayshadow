...
void ParseSegment(std::string_view segment, std::string& mods,
                  std::string& key) {
  mods.clear();
  key.clear();
...
void ExtractCount(std::string_view key, std::string& base_key,
                  std::string& count_part) {
...
void Renderer::Redraw(ClientState& state, struct wl_surface* surface,
  ...
  const int fd = ShmPool::AllocateShmFile(size);
  if (fd == -1) {
    return;
  }
  ...
  cairo_surface_t* cs = cairo_image_surface_create_for_data(
    static_cast<unsigned char*>(data), CAIRO_FORMAT_ARGB32, width, height,
    stride);
  ...
  cairo_set_source_rgba(cr, state.config.bg_color.r, state.config.bg_color.g,
                        state.config.bg_color.b, state.config.bg_color.a);
  cairo_fill(cr);
  ...
  cairo_set_source_rgba(cr, state.config.text_color.r,
                        state.config.text_color.g, state.config.text_color.b,
  ...
  const double max_width = width - kPadding - kRightPadding;
  const double y_pos = ((height - font_extents.height) / 2.0) +
                       font_extents.ascent + kTopBottomPadding - 7.0;
  ...
  for (size_t i = 0; i < segments.size(); ++i) {
    std::string raw_key;
    ParseSegment(segments[i].text, parsed[i].mods, raw_key);
    ExtractCount(raw_key, parsed[i].base_key, parsed[i].count_part);

    cairo_text_extents_t mod_extents;
    cairo_text_extents(cr, parsed[i].mods.c_str(), &mod_extents);
    double w = mod_extents.x_advance;

    if (Icons::IsIconKey(parsed[i].base_key)) {
  ...
  // Determine the starting segment to fit within width constraint
  // (right-aligned scrolling)
  size_t start_seg = 0;
  ...
  double current_x = width - kRightPadding - width_so_far;
  current_x = std::max(current_x, kPadding);

  // Render segments
  for (size_t i = start_seg; i < parsed.size(); ++i) {
    const bool is_last = (i == parsed.size() - 1);
    const Color draw_color = (is_last && state.use_combo_color)
                               ? state.current_combo_color
                               : state.config.text_color;

    cairo_set_source_rgba(cr, draw_color.r, draw_color.g, draw_color.b,
                          draw_color.a);
    cairo_set_font_size(cr, state.config.font_size);
    ...
    if (parsed[i].is_icon) {
      Icons::Draw(cr, parsed[i].base_key, current_x, y_pos, icon_size,
                  draw_color);
      if (parsed[i].base_key == "Space") {
  ...
  const bool is_mouse_active = state.mouse.lmb || state.mouse.rmb ||
                               state.mouse.mmb || state.mouse.back ||
                               state.mouse.forward || state.mouse.has_click;
  if (is_mouse_active) {
    const bool draw_lmb =
      state.mouse.lmb || (state.mouse.has_click && state.mouse.last_lmb);
    const bool draw_rmb =
      state.mouse.rmb || (state.mouse.has_click && state.mouse.last_rmb);
    const bool draw_mmb =
      state.mouse.mmb || (state.mouse.has_click && state.mouse.last_mmb);
    ...
    bool any_button_held = state.mouse.lmb || state.mouse.rmb ||
                           state.mouse.mmb || state.mouse.back ||
                           state.mouse.forward;
    bool is_held = false;
    if (any_button_held && state.mouse.hold_start_time.tv_sec != 0) {
      ...
      const int64_t elapsed_ms =
        ((now.tv_sec - state.mouse.hold_start_time.tv_sec) * 1000L) +
    ...
    if (show_mouse_icon) {
      ...
      const double text_y =
        icon_y + (icon_h * 0.5) + (text_ext.height * 0.5) - 1.0;

      Icons::DrawMouse(cr, start_x, icon_y, icon_w, icon_h, draw_lmb, draw_rmb,
                       draw_mmb, state.mouse.lmb, state.mouse.rmb,
                       state.mouse.mmb, state.config.text_color,
                       state.config.text_color, state.config.hold_color);

      cairo_set_source_rgba(
        cr, state.config.text_color.r, state.config.text_color.g,
        state.config.text_color.b, state.config.text_color.a);
