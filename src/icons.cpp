...
bool Icons::IsIconKey(std::string_view key) noexcept {
  if (key == "Enter" || key == "Left" || key == "Right" || key == "Up" ||
      key == "Down" || key == "Tab" || key == "Space" || key == "Del" ||
      key == "Delete" || key == "Caps" || key == "Home" || key == "End" ||
      key == "PgUp" || key == "PgDn" || key == "Play" || key == "Pause" ||
      key == "Prev" || key == "Next" || key == "Vol+" || key == "Vol-" ||
      key == "Bri+" || key == "Bri-" || key == "Mute") {
    return true;
  }
...
void Icons::Draw(cairo_t* cr, std::string_view key_name, double x, double y,
  ...
  if (key_name == "Enter") {
    ...
    cairo_stroke(cr);
  } else if (key_name.starts_with('F') && key_name.length() >= 2 &&
             key_name.length() <= 3) {
    const double cx = x + (size * 0.5);
    const double cy = y - (size * 0.3);
...
void Icons::DrawMouse(cairo_t* cr, double x, double y, double width,
  ...
  if (lmb) {
    ...
    cairo_arc_negative(cr, cx, y_wt + rw, rw, -std::numbers::pi / 2.0,
                       -std::numbers::pi);
    cairo_line_to(cr, x_wl, y_wb - rw);
    cairo_arc_negative(cr, cx, y_wb - rw, rw, std::numbers::pi,
                       std::numbers::pi / 2.0);
  ...
  // 2. Stroke outlines
  cairo_set_source_rgba(cr, stroke_color.r, stroke_color.g, stroke_color.b,
                        stroke_color.a);
