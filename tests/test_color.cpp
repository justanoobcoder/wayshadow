...
extern void TestColorParsing() {
  ...
  // 6-digit hex
  auto c1 = wayshadow::Color::FromHex("#ff0000");
  assert(c1.has_value());
  ...
  // 6-digit hex without #
  auto c2 = wayshadow::Color::FromHex("00ff00");
  assert(c2.has_value());
  ...
  auto c3 = wayshadow::Color::FromHex("#0000ff80");
  assert(c3.has_value());
  ...
  // Invalid hex
  auto c4 = wayshadow::Color::FromHex("#12345");
  assert(!c4.has_value());

  auto c5 = wayshadow::Color::FromHex("invalid");
  assert(!c5.has_value());
