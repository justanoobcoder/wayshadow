...
void test_color_parsing() {
    ...
    // 6-digit hex
    auto c1 = keypop::Color::from_hex("#ff0000");
    assert(c1.has_value());
    ...
    // 6-digit hex without #
    auto c2 = keypop::Color::from_hex("00ff00");
    assert(c2.has_value());
    ...
    auto c3 = keypop::Color::from_hex("#0000ff80");
    assert(c3.has_value());
    ...
    // Invalid hex
    auto c4 = keypop::Color::from_hex("#12345");
    assert(!c4.has_value());

    auto c5 = keypop::Color::from_hex("invalid");
    assert(!c5.has_value());
